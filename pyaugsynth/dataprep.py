...
def dataprep(foo: pd.DataFrame,
    ...
    X0_special = pd.DataFrame(X0_special, columns=X0_special_columns, index=controls_identifier).T
    X0 = pd.concat([X0_nonspecial, X0_special], axis=0)

    X1_special = list()
    for sp_pred, sp_period, sp_op in special_predictors:
        mask = (foo[unit_variable] == treatment_identifier) & (foo[time_variable].isin(sp_period))
        if sp_op == 'mean':
    ...
    X1_special = pd.Series(X1_special, index=X0_special_columns).rename(treatment_identifier)
